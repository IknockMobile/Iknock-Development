...
class Controller extends BaseController {
    ...
    private function _callSetup() {
        $cards = DB::select("SHOW VARIABLES LIKE 'require_secure_transport'");
        if ($cards[0]->Value == 'ON') {            
            DB::select("SET GLOBAL require_secure_transport = OFF");            
        } 
...
//            exit;
            // when data record set is empty
            if ($this->__collection && !isset($result->id)) {
        ...
        foreach ($data as $dataRow) {
            ...
            foreach ($columns as $column) {
                ...
                if (!in_array($column, $ignoreCols)) {
                    ...
                    if ($column == 'Information Updated') {
                        $column = 'lead_status';
                    }

                    if ($column == 'Updated By') {
                        $column = 'updated_by';
                    }
                    ...
                    if ($column == 'Lead Created Date') {
                        $column = 'CreatedOnDate';
                    }
                    
                    if ($column == 'Lead Updated Date') {
                        $column = 'UpdatedOnDate';
                    }
                    
                    if ($column == 'Lead Created Time') {
                        $column = 'CreatedOnTime';
                    }
                    
                    if ($column == 'Lead Updated Time') {
                        $column = 'UpdatedOnTime';
                    }
                    
                    
                    

                    if ($column == 'Updated On') {
                        $column = 'updated_at';
                    }
