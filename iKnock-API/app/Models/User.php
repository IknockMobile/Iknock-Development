...
class User extends Model implements Auditable {
    ...
    public static function createAgent($user) {
        ...
        $input['image_url'] = empty($user['system_image_url']) ? '' : $user['system_image_url'];
        $input['date_of_join'] = $user['date_of_join'];
        $input['mobile_no'] = $user['mobile_no'];
        /* $input['age']           = $user['age'];
          $input['gender']        = $user['gender']; */
        $input['password'] = $user['password'];
        $input['user_group_id'] = $user['user_group_id'];
        $input['company_id'] = $user['company_id'];
        $input['startup_paid'] = $user['startup_paid'];
        $input['startup_reimbursed'] = $user['startup_reimbursed'];
        $input['forgot_password_hash'] = $user['forgot_password_hash'];
        $input['forgot_password_hash_date'] = $user['forgot_password_hash_date'];
        $input['token'] = self::getToken();
        /* $input['device_type']   = $user['device_type'];
          $input['device_token']  = $user['device_token'];
          $input['device']        = $user['device']; */
    ...
    public static function getTenantUserList($param, $is_active = false) {
        ...
        $query->where('user_group_id', '!=', 4);

        if (isset($param['user_status_id']) AND $param['user_status_id'] != 'all') {
            if ($param['user_status_id'] == '1') {
                $query->where('status_id', '=', 1);
            } else {
        ...
        if (!empty($param['type'])) {
            $query->whereIn('user_group_id', $param['type']); // agent user group id
        }
    ...
    public static function getTenantUserListSub($param) {
        $order_by_map['name'] = 'first_name';
        $order_by_map['email'] = 'email';
        $order_by_map['contact_number'] = 'mobile_no';
        $order_by_map['joining_date'] = 'date_of_join';
