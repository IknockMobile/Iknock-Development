...
//        print_r($params);
// die;
        $query = self::select('lead_detail.*','user_lead_appointment.appointment_date','user_lead_appointment.appointment_end_date','user_lead_appointment.result as appointment_result','user_lead_appointment.id as appointment_id','user_lead_appointment.is_out_bound');
        $query->leftJoin('lead_detail', function($join) use ($params){
            $join->on('lead_detail.id', 'user_lead_appointment.lead_id');
...
//        $query->where('user_lead_appointment.user_id', $params['user_id']);
        ...
        if(isset($params['API']) || isset($params['ignore_out_bound']))
            return $query->paginate(Config::get('constants.EXPORT_PAGE_SIZE'));
    ...
    {
        $result = \DB::select("SELECT user_lead_appointment.*, concat(user.first_name, '', user.last_name) as user_name, lead_detail.title FROM user_lead_appointment 
                              LEFT JOIN `user` ON `user`.id = user_lead_appointment.user_id
                              LEFT JOIN lead_detail ON lead_detail.id = user_lead_appointment.lead_id 
