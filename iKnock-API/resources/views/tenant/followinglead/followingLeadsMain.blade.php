<table class="table lead-table table-bordered">
    ...
    @forelse($followUpLeadViewSetps as $key=>$followUpLeadViewSetp)
            <th class="sort-active text-center show-th  {{ $followUpLeadViewSetp->title_slug == 'notes_and_actions' ? 'note-width':''}}" id="table-th-{{ $key }}">
                ...
                Action
                @elseif($followUpLeadViewSetp->title_slug == 'is_marketing' OR
                    $followUpLeadViewSetp->title_slug == 'is_purchase' OR
                    $followUpLeadViewSetp->title_slug == 'status' OR
                    $followUpLeadViewSetp->title_slug == 'is_retired'                     
                )
                   {{ $followUpLeadViewSetp->title }}
                @else
                <!--{{ $followUpLeadViewSetp->title }}-->
                @if($input['sort_type'] == 'asc' || request()->get('sort_type') == 'asc')
                    <span class="sort-columns" data-type="desc" data-toggle="tooltip" data-placement="top" title="Desc" data-id="{{ $followUpLeadViewSetp->id }}" data-column="{{ $followUpLeadViewSetp->id }}">
                        <span>{{ $followUpLeadViewSetp->title }}</span>
                        <i class="fas fa-sort-down"></i>
                    </span>
                    @else
                    <span class="sort-columns" data-type="asc" data-toggle="tooltip" data-placement="top" title="Asc" data-id="{{ $followUpLeadViewSetp->id }}" data-column="{{ $followUpLeadViewSetp->id }}">
                        <span>{{ $followUpLeadViewSetp->title }}</span>
                        <i class="fas fa-sort-up"></i>
                    </span>
                    @endif
