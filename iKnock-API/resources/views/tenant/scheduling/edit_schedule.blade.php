...
<div class="right_col" role="main">
    <div class="row" id="content-heading">
        ...
        <div class="col-md-4 text-right">
            <?php
            if (isset($data['detail']->lead_id) AND $data['detail']->lead_id != 0) {
                ?>
            <a class="btn" href="{{ URL::to('tenant/lead/edit') }}/{{$data['detail']->lead_id}}">Lead View</a>
            <?php } ?>
        ...
        <div class="col-md-8">
            ...
            <form method="post" enctype="multipart/form-data">
                ...
                <div class="form-group mt-40">
                    <label>Note</label>
                    <textarea name="note" class="input" rows="5">{!! $data['detail']->result ?? '' !!}</textarea>
                </div>
                <div class="form-group mt-10">
                    <label>Person With Whom You Are Meeting:</label>
                    <input name="person_meeting" type="text" value="{{ $data['detail']->person_meeting ?? '' }}" class="input" >
                </div>
                <div class="form-group mt-10">
                    <label>Phone:</label>
                    <input name="phone" type="text" value="{{$data['detail']->phone ?? '' }}" class="input" >
                </div>
                <div class="form-group mt-10">
                    <label>Email:</label>
                    <input name="email" type="text" value="{{$data['detail']->email ?? ''}}" class="input" >
                </div>
                <div class="form-group mt-10">
                    <label>Additional Notes:</label>
                    <input name="additional_notes" type="text" value="{{$data['detail']->additional_notes ?? ''}}" class="input" >
                </div>
                <div class="form-group">
