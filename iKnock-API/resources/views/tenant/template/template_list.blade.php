...
$(document).ready(function(){
    ...
    $(document).on('click', '.delete', function (e) {
     ...
     $(document).on('click', '.copy', function (e) {
            ...
            copyTempleteForm(id, get_template_id);
           
            return false;
        });

     function copyTempleteForm(id, get_template_id){
         var templateValue = prompt("Please enter an copy template name");
         
         if(templateValue != null){
                if(templateValue != ''){
                    var choice = confirm('Do you really want to copy this template?');
                    if (choice === true) {
                            let copyRecord = "{{ URL::to('/tenant/template/copy') }}" + "/" + id;
                            var data = {id: id, template_name:templateValue};
                            ajaxCall('GET', copyRecord, data, {});
                            var redirect_url = '';
                            redirect_url = typeof redirect_url == '' ? window.location.href : redirect_url;
                            setTimeout(function () {
                                window.location.href = redirect_url;
