...
<div class="right_col" role="main">
	<div class="container body">
		<div class="col-md-12">
			<div class="cust-head"><i class="fas fa-list"></i> Follow Up Lead View Setup</div>
		</div>
		<div class="col-md-12 mt-5">
			<div class="panel panel-default">
				<div class="panel-heading">
					<div class="row">
						<div class="col-md-6">
							<h5>Follow Up Lead View Setup</h5>
						</div>
						<div class="col-md-6 text-right">
							<a href="{{ route('tenant.follow-leadview.create') }}" class="btn btn-primary btn-sm"><i class="fas fa-plus"></i></a>
						</div>
					</div>
				</div>
					...
					<table class="table table-bordered">
						...
						<tbody>
							@forelse($followUpLeadViewSetps as $key => $followUpLeadViewSetp)
								<tr id="{{ $followUpLeadViewSetp->id }}">
									<td>{{ ++$key }}</td>
									<td>{{ $followUpLeadViewSetp->title }}</td>
									<td><input type="checkbox" class="setupview" {{ $followUpLeadViewSetp->is_show == 1 ? 'checked':''  }} data-toggle="toggle" data-id="{{ $followUpLeadViewSetp->id }}"  data-offstyle="danger" data-size="sm"> </td>
									<td>
										<a href="{{ route('tenant.follow-leadview.edit',$followUpLeadViewSetp->id) }}" class="btn btn-primary btn-sm"><i class="fas fa-pen"></i></a>
										<a href="#" data-action="{{ route('tenant.follow-leadview.destroy',$followUpLeadViewSetp->id) }}" data-id="{{ $followStatus->id }}" class="btn btn-danger btn-sm remove-crud "><i class="fas fa-trash"></i></a>
									</td>
								</tr>
							@empty
								<tr>
