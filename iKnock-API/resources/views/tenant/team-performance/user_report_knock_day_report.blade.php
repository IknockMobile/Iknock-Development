...
<div class="right_col" role="main">
    <div class="row" id="content-heading">
        <div class="col-md-8">
            <div class="dropdown">
                <button class="btn  dropdown-toggle add-bt" type="button" data-toggle="dropdown" style="background: transparent;">
                    <span class="cust-head">Best Time Of Day Knocks Statistics Report</span>
        ...
        <div class="col-md-3 text-right" style="margin-top:6px;">
            <button class="b1" id="export-btn">All status data Export</button>
            <button class="b1" id="export-btn-new">Answered and No Answer Export</button>
        </div>
    </div>
        ...
        <form class="comm_form">
            ...
            <div class="col-md-2 form-group">
                ...
                <select class="form-control summary datetype" name="timetype">
                    <option value="day_part" selected="">Day Part</option>
                </select>
            </div>
            <div class="col-md-2 form-group">
    ...
    <div id="chartWidth"></div>
    <div id="newChartWidth"></div>
        ...
        <div class="col-md-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    <div class="row" id="content-heading">
                        <div class="col-md-10">
                            <h5>Hourly Response Status : All Status Stacked Bar Chart</h5>
                        </div>
            ...
            <div class="panel panel-default">
                <div class="panel-heading">
                    <div class="row" id="content-heading">
                        <div class="col-md-10">
                            <h5>Hourly Response Status: Answered vs. No Answer Stacked Bar Chart</h5>
                        </div>
                        <div class="col-md-2 text-right">
                            <div onclick="openFullscreenBar();"><i class="fa fa-arrows-alt"></i></div>
                        </div>
                ...
                <div class="panel-body">                        
                    <div id="newChartContainer" style="height: 600px; margin: 0 auto"></div>
                </div>
            </div>
        </div>
    ...
    <div class="row " id="pg-content">
        <div class="panel panel-default">
            <div class="panel-heading">
                <div class="row" id="content-heading">
                    <div class="col-md-10">
                        <h5>Hourly Response Status : All Status Table Data</h5>
                    </div>
                    <div class="col-md-2 text-right">
                        <div onclick="openFullscreenBar();"><i class="fa fa-arrows-alt"></i></div>
                    </div>
        ...
        <div class="panel panel-default">
            <div class="panel-heading">
                <div class="row" id="content-heading">
                    <div class="col-md-10">
                        <h5>Hourly Response Status: Answered vs. No Answer Table Data</h5>
                    </div>
                    <div class="col-md-2 text-right">
                        <div onclick="openFullscreenBar();"><i class="fa fa-arrows-alt"></i></div>
                    </div>
            ...
            <div class="panel-body" style="overflow-x: auto;">                        
                <table class="table table-bordered">
                    <thead id="new_thead_month">
                    </thead>
                    <tbody id="new_tbody_month">
                    </tbody>
                </table>
            </div>
        </div>                
...
<script type="text/javascript">
    $(document).ready(function () {
        function ColChart(user_names, status, types) {
            ...
            var total_width = $('#chartWidth').width() - 56;
            var chart = new CanvasJS.Chart("chartContainer", {
                ...
                width: total_width,
                title: {
                    text: "Hourly Response Status : All Status Stacked Bar Chart",
                    fontSize: 12,
                },
                axisY: {
                    title: "Number of Knocks",
                    includeZero: true,
                    crosshair: {
                        enabled: true
                    },
        ...
        function ColChartNew(user_names, status, new_types) {
            Highcharts.chart('container', {
                chart: {
                    type: 'column',
                    events: {
                ...
                xAxis: {
                    ...
                    categories: user_names,
                    title: {
                        text: 'Bar Chart FollowUp Lead Status Report13',
                    },
            ...
            var tatol_width = $('#newChartWidth').width() - 56;

            var chartnew = new CanvasJS.Chart("newChartContainer", {
                animationEnabled: true,
                theme: "light2",
                height: 600,
                width: tatol_width,
                title: {
                    text: "Hourly Response Status: Answered vs. No Answer Stacked Bar Chart",
                    fontSize: 12,
                },
                axisY: {
                    title: "Number of Knocks",
        ...
        function ajaxRequest(method, url, data = {}) {
            ajaxCall(method, url, data).then(function (res) {
                ...
                if (res.code == 200) {
                    ...
                    var month_status_new = record.month_status_new;
                    var types = record.types;
                    var new_types = record.new_types;
                    var months = record.months;

                    ColChart(user_names, status, types);
                    ColChartNew(user_names, status, new_types);
                    ...
                    if (months.length > 0) {
                        ...
                        thead += '</tr>';
                        $('#new_thead_month').html(thead);
                    }
                    if (month_status_new.length > 0) {
                        var tbody2 = '';
                        var index = 1;
                        for (var i = 0; i < month_status_new.length; i++) {
                            tbody2 += '<tr>';
                            tbody2 += '<td>' + index + '</td>';
                            tbody2 += '<td>' + month_status_new[i].name + '</td>';
                            if (month_status_new[i].data.length > 0) {
                                var type = $('.value').val();
                                for (var a = 0; a < months.length; a++) {
                                    if (month_status_new[i].data[a]) {
                                        tbody2 += '<td>' + month_status_new[i].data[a] + '</td>';
                                    } else {
                                        tbody2 += '<td>0</td>';
                                    }
                                }
                        ...
                        $('#new_tbody_month').html(tbody2);
                    }
                } else {
                    var errorMessage = res.data[0].error_message;
        ...
        $(window).load(function () {
            ajaxRequest('GET', base_url + "/tenant/lead/status/user/report/knock/withcolour", data);
        });
        ...
        $('.save').click(function (e) {
            ...
            $('#chartContainer').html('<h1><div class="row"><div class="col-md-12 text-center"><img src="{{ asset('image/loder.gif') }}" width="100px"></div></div>');
            $('#newChartContainer').html('<h1><div class="row"><div class="col-md-12 text-center"><img src="{{ asset('image/loder.gif') }}" width="100px"></div></div>');
            $('#tbody_month').html('<h1><div class="row"><div class="col-md-12 text-center"><img src="{{ asset('image/loder.gif') }}" width="100px"></div></div>');
            $('#new_tbody_month').html('<h1><div class="row"><div class="col-md-12 text-center"><img src="{{ asset('image/loder.gif') }}" width="100px"></div></div>');
            ajaxRequest('GET', base_url + "/tenant/lead/status/user/report/knock/withcolour", data);
        ...
        $(document).on('click', '#export-btn', function () {
            ...
            var type = $('.value').val();
            var datetype = $('.datetype').val();
            var data = {type_id: type_id, type: type, start_date: start_date, end_date: end_date, export: true, datetype: datetype};
            console.log('data', data);
            var qString = $.param(data);
            var url = "{{URL::to('/tenant/lead/status/user/report/knock/withcolour?')}}" + qString;
            document.location.href = url;
        });

        $(document).on('click', '#export-btn-new', function () {
            var type_id = $('.type_list').selectpicker('val');
            if (Array.isArray(type_id)) {
                type_id = type_id.join();
            }
            ...
            var type = $('.value').val();
            var datetype = $('.datetype').val();
            var data = {type_id: type_id, type: type, start_date: start_date, end_date: end_date, export_new: true, datetype: datetype};
            var qString = $.param(data);
            var url = "{{URL::to('/tenant/lead/status/user/report/knock/withcolour?')}}" + qString;
            document.location.href = url;
        });
